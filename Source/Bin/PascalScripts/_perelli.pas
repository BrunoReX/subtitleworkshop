...
// Mod by Godem - sept.2011
// http://feliratok.info
// http://felirat.blog.hu/
...
const
  ...
  CPSlimit = 15;  //Change as you see fit.
  CRLF = ''#13''#10;
  durmin = 1500; //1,501 sec
...
begin
  ...
  begin
	...
	if ((cps > CPSlimit) and (duration < durmin)) then
    begin
      subtrans := '[*' + IntToStr(cps) + ' kar/s*]' + subtrans;
      j := j+1;
    end;
 ...
 MsgBox(IntToStr(j) + ' felirattábla CPS-e magasabb, mint ' + IntToStr(CPSlimit) + ' kar/s.' + CRLF + 'Feljegyezve a Fordításban.', '&Ok', '', '', $20);
end.
