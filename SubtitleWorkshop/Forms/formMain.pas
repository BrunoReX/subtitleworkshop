...
begin
  ...
  ActualLangFile := ExtractFilePath(Application.ExeName) + ID_LANGDIR + '\' + ActualLangFile;
...
begin
  OCRDefFile := ExtractFilePath(Application.ExeName) + ID_OCRDIR + '\' + (Sender as TMenuItem).Caption + ID_OCREXT;
...
begin
  ...
  try
    i := FindFirst(ExtractFilePath(Application.ExeName) + ID_LANGDIR + '\*.lng', faAnyFile, Busca);
    while i = 0 do
    begin
...
begin
  ...
  try
    i := FindFirst(ExtractFilePath(Application.ExeName) + ID_OCRDIR + '\*' + ID_OCREXT, faAnyFile, Busca);
    while i = 0 do
    begin
...
begin
  Scripts := TStringList.Create;
  try
    i := FindFirst(ExtractFilePath(Application.ExeName) + ID_PASCALDIR + '\*.pas', faAnyFile, Busca);
    while i = 0 do
    begin
      Scripts.Add(Copy(Busca.Name, 1, Length(Busca.Name)-4));
      i := FindNext(Busca);
...
begin
  ...
  try
    ...
    OCRDefFile                  := Ini.ReadString('Information and Errors', 'OCR Definitions file', ExtractFilePath(Application.ExeName) + ID_OCRDIR + '\Default' + ID_OCREXT);
  ...
  Ini := TIniFile.Create(ExtractFilePath(Application.ExeName) + ID_INIOUTNAME);
  try
    //added by adenry: begin
    // --------------------------- //
...
begin
  ...
  try
    ...
    psCompExec.Script.LoadFromFile(ExtractFilePath(Application.ExeName) + ID_PASCALDIR + '\' + (Sender as TMenuItem).Caption + '.pas'); //StripHotKey added by adenry for toolbar button fix
...
begin
  OCRDefFile := ExtractFilePath(Application.ExeName) + ID_OCRDIR + '\' + cmbOCRScripts.Items[cmbOCRScripts.ItemIndex] + ID_OCREXT;
  //added by adenry later:
  if ((ErrorsToCheck.eOCRErrors) or (ErrorsToFix.eOCRErrors)) and (FileExists(OCRDefFile)) then
  begin
    ParseOCRErrors(OCRDefFile);
...
begin
  ...
  begin
    for i := 2 to mnuLanguage.Count - 1 do
      ...
      begin
        Screen.Cursor := crHourglass;
        eng := TMemIniFile.Create(ExtractFilePath(Application.ExeName) + ID_LANGDIR + '\English.lng');
        try
          ...
          for ini := 2 to mnuLanguage.Count - 1 do
            ...
            begin
              ...
              lang := TMemIniFile.Create(ExtractFilePath(Application.ExeName) + ID_LANGDIR + '\' + mnuLanguage.Items[ini].Caption + '.lng');
              try
