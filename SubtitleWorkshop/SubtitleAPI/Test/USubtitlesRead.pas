...
uses
  USubtitleFile, USubtitlesFunctions, SysUtils, FastStrings, dialogs {$IFDEF VIPLAY}, Forms{$ENDIF};
...
const
  TSubtitleFormatsName : array[1..40] of String =
...
begin
  ...
  try
    ...
    // Add #13#10 as needed
    for i := Length(BigStr)-1 downto 2 do
    begin
      Text := Copy(BigStr, i, 4);
      if (Text = 'BG @') or
         (Copy(Text, 1, 2) = '#T') then
    ...
    begin
      if (Pos('BG @', tmpSubFile[i+1]) = 1) and
         (IsInteger(Copy(tmpSubFile[i], Pos('#T', tmpSubFile[i]) + 3, 14))) then
      begin
        Text := '';
        c    := 1;
        ...
        begin
          ...
          if Text <> '' then
            Text := Text + #13#10 + Copy(tmpSubFile[i+c], a, Length(tmpSubFile[i+c]))
          else
            Text := Copy(tmpSubFile[i+c], a, Length(tmpSubFile[i+c]));
          Inc(c);
        end;
        // The format of the time is: hmmsszzz without ":"
        tmpSubFile[i]   := Copy(tmpSubFile[i], 3, 8);
        tmpSubFile[i+c] := Copy(tmpSubFile[i+c], 3, 8);

        InitialTime := ((StrToIntDef(Copy(tmpSubFile[i], 1, 1), 0)*3600)*1000) + ((StrToIntDef(Copy(tmpSubFile[i], 2, 2), 0)*60)*1000) + (StrToIntDef(Copy(tmpSubFile[i], 4, 2), 0)*1000) + StrToIntDef(Copy(tmpSubFile[i], 6, 3), 0);
        if (IsInteger(tmpSubFile[i+c])) then
          FinalTime := ((StrToIntDef(Copy(tmpSubFile[i+c], 1, 1), 0)*3600)*1000) + ((StrToIntDef(Copy(tmpSubFile[i+c], 2, 2), 0)*60)*1000) + (StrToIntDef(Copy(tmpSubFile[i+c], 4, 2), 0)*1000) + StrToIntDef(Copy(tmpSubFile[i+c], 6, 3), 0)
        else
          FinalTime := InitialTime + 2000;
...
begin
  ...
  try
    ...
    begin
      if a < i then
        Delete(BigStr, a, 1) else
        Delete(BigStr, i, a-i+3);
      i := Pos('<!--', BigStr);
      a := Pos('-->', BigStr);
    end;
...
begin
  ...
  showmessage(tmpSubFile.Text);

  tmpSubFile.Text := RemoveRTFFormatting(tmpSubFile.Text);
